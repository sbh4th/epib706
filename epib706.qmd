...
quarto::quarto_render("epid706.qmd") 
-->
```
...
# 8 Course Outline (12 "questions" to consider)
...
The Lau paper et al. is more of a grand overview and (perhaps failed) attempt to identify different paradigms of research in epidemiology's history, with an eye toward the future and what it *should* look like. I think this is more of a perspective from senior researchers that may not have grown up with the kind of methodological training you are receiving, and who may be feeling this notion (you'll see this a lot) that the field is 'too focused on methods' rather than substantive inquiry. I think it is useful for you to be able to articulate a defense and counter-argument to that assertion, and to think about what kind of discipline you think epidemiology should strive to become in the future.

### `r classdate(w1c2,1)`: Reflections on the past
...
### `r classdate(w1c1,2)`: Where does epidemiology stand today?
...
### `r classdate(w1c1,3)`: Does it matter whether questions and methods align?
...
### `r classdate(w1c2,3)`: Are potential outcomes and DAGs necessary?
...
### `r classdate(w1c1,4)`: Are well-defined interventions needed for causal questions?
...
### `r classdate(w1c2,4)`: Are non-manipulable exposures causes?
...
### `r classdate(w1c1,5)`: Case study: racial bias in policing.
...
### `r classdate(w1c2,5)`: Are RCT's special?
...
### `r classdate(w1c1,6)`: What if we can't randomize?
...
### `r classdate(w1c2,6)`: Should our studies be representative?
...
### `r classdate(w1c2,7)`: How should we use p-values?
...
### `r classdate(w1c1,8)`: What are alternatives to p-values?
...
- Hamra GB, Stang A, Poole C. The researcher and the consultant: from testing to probability statements. *Eur J Epidemiol* 2013;28:939–944. (https://doi.org/10.1007/s10654-013-9861-4)

## Week 8: No classes [Winter Break]
...
### `r classdate(w1c2,9)`: How can we quantify our assumptions?
...
### `r classdate(w1c1,10)`: E-values, worthwhile or worthless?
...
### `r classdate(w1c1,11)`: How should we think about generalizing to specific individuals?
...
### `r classdate(w1c1,13)`: How do we deal with uncertainty?  
...
- Blastland M et al. Five rules for evidence communication. *Nature* 2020;587:362–364 (https://doi.org/10.1038/d41586-020-03189-1) [2664 words]
...
### `r classdate(w1c2,13)`: Debate on epidemiology's aspirations and capabilities
